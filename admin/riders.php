...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adjust_score'])) {
    ...
    if ($rider_id > 0 && $new_score >= 0 && $new_score <= 100 && !empty($reason)) {
        ...
        $edit_success = 'Score updated. New status: ' . strtoupper($new_status) . ' (' . $new_score . '/100). Reason: ' . $reason;
    } else {
...
<?php if ($edit_rider): ?>
    <div class="gov-card" style="max-width:500px;">
        <div class="gov-card-header">
            <div>
                <div class="gov-card-title">Score Adjustment — <?php echo sanitize($edit_rider['full_name']); ?></div>
        ...
        <form method="POST" action="">
            ...
            <button type="submit" class="gov-btn gov-btn-primary">Update Score</button>
        </form>
    </div>
<?php endif; ?>
